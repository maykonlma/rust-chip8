...
impl Cpu {
    ...
    fn play_beep(&self) {
        ...
        let device = host.default_output_device().expect("Falha ao encontrar o dispositivo de saída.");
        ...
        let stream = device.build_output_stream(
            &config,
            move |output_buffer: &mut [f32], _output_callback_info: &OutputCallbackInfo| {
                
                for sample in output_buffer.iter_mut() {
                    let time = sample_position as f32 / sample_rate;
                    let value = (2.0 * std::f32::consts::PI * frequency * time).sin();
                    *sample = value;
                    sample_position += 1;
                }
            },
            move |err| eprintln!("Erro no stream de áudio: {}.", err),
            None,
        ).expect("Falha ao criar stream de áudio.");
        
        stream.play().expect("Falha ao reproduzir fluxo de áudio.");
